...
PIPELINE = 'moderation'
dataset_path = 'train_test_data/harmful_helpful_train_set.csv' #'train_test_data/relevant_irrelevant.csv'  
ctrl_str = ctrl_str = ["[Harmful]",'[Helpful]'] #ctrl_str =["[Irrelevant]", "[Relevant]"] 
...
def pos_logit_to_reward(logit, task):
    ...
    for i in range(len(logit)):
        if task[i] == "[Irrelevant]" if PIPELINE == 'topical' else "[Harmful]":
            logit[i] = -logit[i]
        elif task[i] == "[Relevant]" if PIPELINE == 'topical' else "[Helpful]":
            pass
        else:
...
def main():
    ...
    if not os.path.isdir(model_path):
        ...
        # Two labels relevant and irrelevant dataset
        ...
        if not os.path.isdir(classifier_path):
            ...
            generation_kwargs = {
                ...
                "eos_token_id": model_tokenizer.eos_token_id, # -1
            }
